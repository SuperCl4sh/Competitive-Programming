...
int main() {
	...
	ll dp[N+1][N+1];
	rep (i,1,N+1) rep (j,1,N+1) dp[i][j]=(ll)(1e13);
	ll psa[N+1];
	psa[0]=0LL;
	rep (i,1,N+1) dp[i][i]=ar[i],psa[i]=psa[i-1]+ar[i];
	
	ROF (i,N,0,1) {
		rep (j,i+1,N+1) {
			rep (k,i,j+1) {
				if (k+1<=j) dp[i][j]=min(dp[i][j],dp[i][k]+dp[k+1][j]+psa[j]-psa[i-1]);
			}
		}
	}
	outln(dp[1][N]-psa[N]-psa[0]);
	rtn 0;
}
