...
int main() {
	...
	vt<ll>arr(n+1);
	ll total=0;
	rep (i,1,n+1) {read(arr[i]);total+=arr[i];}
	if (n==2) {
		outln(abs(arr[1]-arr[2]),abs(arr[1]-arr[2]));
		rtn 0;
	}
	rep (i,1,n+1) {
		ar.pb(arr[i]);
	...
	psa[0]=0;
	rep (i,1,sz(ar)) psa[i]=psa[i-1]+ar[i]; 

	rep (i,1,n+1) {
		int endOfRange=i+n,left=i,right=endOfRange,mid;ll ans=INT_MAX;
		while (right-left>1) {
			mid=(left+right)/2;
			ll calc1=calc(i,mid),calc2=total-calc1,calc3=calc(i,mid+1),calc4=total-calc3,diff=abs(calc1-calc2),diff2=abs(calc3-calc4);
			if (diff>diff2) left=mid;
			else right=mid;
			ans=min({ans,diff,diff2});
		}
		out(ans);
		if (i==n) out('\n');
		else out(' ');	
