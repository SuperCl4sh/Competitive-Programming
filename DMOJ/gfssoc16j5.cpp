//https://dmoj.ca/problem/gfssoc16j5
#include <bits/stdc++.h>
using namespace std;
/*
...
clock_t start;
/*--------------------------------------------------------------PROGRAM START-------------------------------------------------------------------------*/
int ar[(int)(1e5)+1][4],dp[1<<4][(int)(1e5)+1],n;
int solve(int ind, int mask) {
	if (ind==n) rtn 0;
	else if (dp[mask][ind]!=-1) rtn dp[mask][ind];
        int ret=0;
	rep (i,0,4) {
			
		if (!(ar[ind][i]==-1||(mask>>(i))&1)) ret=max(ret,ar[ind][i]+solve(ind+1,mask+(1<<i)));
		ret=max(ret,solve(ind+1,mask));
	}
	rtn dp[mask][ind]=ret;	
...
int main() {
	...
	read(n);
	rep (i,0,n) 
		read(ar[i][0],ar[i][1],ar[i][2],ar[i][3]);
	outln(solve(0,0));
	rtn 0;
}
