...
struct dsu{
	int Find(int a) {
		while (a!=ids[a]) {ids[a]=ids[ids[a]];a=ids[a];}
		rtn a;
	}
	...
	void init() {
		rep (i,0,(int)(1e5)+1) ids[i]=i,sizes[i]=1;
	}
	int sizes[(int)(1e5)+1],ids[(int)(1e5)+1];
}go;
int main() {
	boost;
	int N,Q;read(N,Q);
	...
	rep (_,0,Q) {
		read(query,X,Y);
		if (query=='A') go.Union(X,Y);
		else outln("YN"[go.Find(X)!=go.Find(Y)]);
	}
