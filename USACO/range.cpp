...
int main() {
	...
	rep (i,1,n+1) {
		rep (j,1,n+1) {
			psa[i][j]=psa[i-1][j]+psa[i][j-1]-psa[i-1][j-1]+(grid[i][j]-'0');
		}
	}
	...
	rep (i,2,n+1) {
		...
		for (int j=1;j+i-1<=n;j++) {
			for (int k=1;k+i-1<=n;k++) {
				int x1=j,y1=k,x2=j,y2=k+i-1,x3=j+i-1,y3=k+i-1,x4=j+i-1,y4=k;
				ans+=psa[x3][y3]-psa[x4][y4-1]-psa[x2-1][y2]+psa[x1-1][y1-1]==i*i;
