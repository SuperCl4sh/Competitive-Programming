...
int main() {
	...
	int n;read(n);
	fences.resize(n+1);fenceWeights.resize(n+1);
	rep (i,1,n+1) {
		int num,weight,items,items2;read(num,weight,items,items2);
		//assert(num==i); scam
		fenceWeights[num]=weight;
		rep (_,0,items) {
			int a;read(a);
			fences[num].start.pb(a);
		}
		rep (_,0,items2) {
			int b;read(b);
	...
	rep (i,1,n+1) {
		...
		each (a,initialAdj) {mem(used,0);used[a]=1;dfs(a,i,fenceWeights[a]);}
		initialAdj=fences[i].stop;
		each (a,initialAdj) {mem(used,0);used[a]=1;dfs(a,i,fenceWeights[a]);}
