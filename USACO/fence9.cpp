...
int main() {
	...
	a.inter=0.0;
	b.slope=ld(m)/ld(n-p);
	b.inter=ld(m)-b.slope*ld(n);



	int ans=0,currY=1;
	while (currY<=m) {
		ld leftMost=(ld(currY)-a.inter)/a.slope,rightMost=((ld(currY)-b.inter)/b.slope);
	//	outln("Start:",leftMost,rightMost);	
		if (n==p) {
			rightMost=n;
		}
		if (ceil(leftMost)-leftMost<EPS) {
			leftMost=ceil(leftMost)+1;

		}
		if (rightMost-floor(rightMost)<EPS) {
			rightMost=floor(rightMost)-1;
		}
		
		leftMost=ceil(leftMost),rightMost=floor(rightMost);
	//	outln("End:",leftMost,rightMost);
		if (int(leftMost)<=int(rightMost)) ans+=int(rightMost)-int(leftMost)+1;
