...
int main() {
	...
	while (T--) {
		int A;
		read(A);
		int currTot=1,currSize=1;
		while (currTot+2*currSize<=A) {
			currTot+=2*currSize,++currSize;
		}
		currSize+=(currTot!=A);
		int best=currSize;
		currTot=1,currSize=1;
		while (currTot+2*currSize+1<=A) {
			currTot+=2*currSize+1,++currSize;
		}
		currSize+=(currTot!=A);
		best=min(best,currSize);
		outln(best);
	}
	rtn 0;
}
