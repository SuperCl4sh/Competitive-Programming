...
int main() {
	...
	while (T--) {
		...
		if ((N/2)&1) {
			...
			ll curr=1,halved=N/2;
			while (curr<halved) {
				if (((halved-curr)*2*curr)/__gcd(halved-curr,2*curr)<=halved) {
					outln(halved-curr,halved-curr,2*curr);
					break;
				}
				curr<<=1;
			}	
