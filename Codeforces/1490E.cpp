...
int main() {
	...
	while (T--) {
		int N;read(N);
		vector<ll>tmp(N);read(tmp);
		vector<pair<ll,int>>ar;
		rep (i,0,N) ar.pb(mp(tmp[i],i));
		sort(all(ar));
		int l=-1,r=N+1,m;
		while (r-l>1) {
			m=l+r>>1;
			ll curr=0;
			rep (i,0,m+1) curr+=ar[i].F;
			int ptr=m+1;
			while (ptr<N&&curr>=ar[ptr].F) curr+=ar[ptr++].F;
			if (curr>=ar[N-1].F) r=m;
			else l=m;
		}
		vector<int>ans;
		rep (i,r,N) ans.pb(ar[i].S+1);
		sort(all(ans));
		writeln(sz(ans),'\n',ans);
	}
	return 0;
