...
int main() {
	...
	while (T--) {
		...
		int smallestDiff=INT_MAX,val=ar[0];
		rep (i,1,N) {
			if (ar[i]-ar[i-1]<=smallestDiff) smallestDiff=ar[i]-ar[i-1],val=ar[i-1];
		}
		vt<int>ans;
		int cnt=0;
		rep (i,lower_bound(all(ar),val)-ar.begin(),N) {
			if (ar[i]!=val+smallestDiff||cnt) ans.pb(ar[i]);
			cnt+=(ar[i]==val+smallestDiff);
		}
		rep (i,0,lower_bound(all(ar),val)-ar.begin()) ans.pb(ar[i]);ans.pb(val+smallestDiff);
		outln(ans);
	}
	rtn 0;
}
