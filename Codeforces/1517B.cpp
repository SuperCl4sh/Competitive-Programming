...
int main() {
	...
	int T;
	read(T);
	vt<vt<vt<int>>>finalAns;
	rep (_,0,T) {
		int N,M;
		read(N,M);
		vt<vt<int>>paths(N);
		vt<pair<int,pii>>totPath;
		rep (i,0,N) {
			rep (j,0,M) {
				int a;
				read(a);
				paths[i].pb(a);
				totPath.pb(mp(a,mp(i,j)));
			}
		}
		sort(all(totPath));
		mem(used,0);
		vt<int>tempAns(M,0);
		vt<vt<int>>ans;
		rep (i,0,N) ans.pb(tempAns);
		rep (i,0,M) used[totPath[i].S.F][totPath[i].S.S]=1,ans[totPath[i].S.F][i]=totPath[i].F;
		
		rep (i,0,N) {
			rep (j,0,M) {
				if (ans[i][j]) continue;
				rep (k,0,M) {
					if (!used[i][k]) {used[i][k]=1;ans[i][j]=paths[i][k];break;}
				}
			}
		}
		each (checkpoint,ans) outln(checkpoint);
	}
	rtn 0;
}
