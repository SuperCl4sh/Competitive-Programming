...
int main() {
	...
	sort(all(people));
	vt<vt<ll>>groups;
	vt<ll>currGroup={people[0]};
	rep (i,1,N) {
		if (people[i]-people[i-1]>X) {groups.pb(currGroup);currGroup={people[i]};}
		else currGroup.pb(people[i]);
	}
	if (!currGroup.empty()) groups.pb(currGroup);
	vt<ll>requiredStudents;
	rep (i,1,sz(groups)) {
		ll diff=groups[i][0]-groups[i-1][sz(groups[i-1])-1];
		requiredStudents.pb(ceil(ld(diff)/ld(X))-1);
	}
	sort(all(requiredStudents));
	ll ans=sz(groups);
	each (gap, requiredStudents) {
		if (gap<=K) K-=gap,--ans;
		else break;
	}
	outln(ans);
