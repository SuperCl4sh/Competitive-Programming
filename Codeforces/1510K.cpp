...
int main() {
	...
	read(N);
	vt<int>ar(2*N+1);
	rep (i,1,2*N+1) read(ar[i]);
	vt<int>start=ar;
	int used=0,turn=0;bool good=0;
	while (!is_sorted(all(ar))&&used<=2*N) {
		if (turn) {
			rep (i,1,N+1) swap(ar[i],ar[i+N]);
		}
		else {
			FOR (i,1,2*N+1,2) swap(ar[i],ar[i+1]);
		}
		++used,turn^=1;
	}
	good|=is_sorted(all(ar));
	int best=used;
	used=0,turn=1;
	ar=start;
	while (!is_sorted(all(ar))&&used<=2*N) {
