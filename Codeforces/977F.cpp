...
int dp[(int)(2e5)+5];
int solve(int ind) {
	if (dp[ind]!=-1) rtn dp[ind];
	auto it=upper_bound(all(freq[ar[ind]+1]),ind);
	rtn dp[ind]=(it!=freq[ar[ind]+1].end())?1+solve(*it):0;
}
void getPath(int ind) {
	out(ind+1);
	auto it=upper_bound(all(freq[ar[ind]+1]),ind);
	if (it!=freq[ar[ind]+1].end()) {
		out(" ");
		getPath(*it);
	}
	else out('\n');
