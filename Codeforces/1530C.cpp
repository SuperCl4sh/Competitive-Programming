...
int main() {
	...
	rep(_,0,T) {
		...
		ll left=-1,right=(ll)(1e13),mid;
		while (right-left>1) {
			mid=(left+right)>>1;
			ll iter=(mid+N)-ll(floor((mid+N)/4LL)),totA=0,totB=0;
			if (iter) {
				totA=min(mid,iter)*100LL;
				iter-=min(mid,iter);
			}
			rep (i,0,iter) totA+=A[i];
			rep (i,0,min(N,(mid+N)-ll(floor((mid+N)/4LL)))) totB+=B[i];
			if (totA>=totB) right=mid;
			else left=mid;
		}
		outln(right);
	}
