...
string S;
ll dp[2][(int)(2e5)+5];
ll solve(int ind, int dig) {
	if (ind==sz(S)||dig!=S[ind]-'0'&&S[ind]!='?') return 0LL;
	else if (dp[dig][ind]!=-1) return dp[dig][ind];
	return dp[dig][ind]=solve(ind+1,!dig)+1;
}
int main() {
	boost;
	int T;read(T);
