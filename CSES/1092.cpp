...
int main() {
	...
	ll N;
	read(N);
	if (N<=2) {outln("NO");rtn 0;}
	if (((N*(N+1))/2)&1) {outln("NO");rtn 0;}
	vt<int>nums1,nums2;
	if (!(N&1)) {
		rep (i,1,N/2+1) {
			if (sz(nums1)==(N/2)) {nums2.pb(i);nums2.pb(N-i+1);}
			else {nums1.pb(i);nums1.pb(N-i+1);}
		}
		...
		rep (i,1,N/2+1) {
			if (i!=(N+1)/4&&i!=3*(N+1)/4) {
				if (sz(nums1)==int(N/2)-1) {nums2.pb(i);nums2.pb(N-i+1);}
				else {nums1.pb(i);nums1.pb(N-i+1);}
			}
		}
		nums1.pb((N+1)/2);nums1.pb((N+1)/4);
		nums2.pb(3*(N+1)/4);
	}
	outln("YES");
	outln(sz(nums1));
